...
async function deleteBook(
  ...
  const { error } = await supabase.from('user_reading_list')
    .delete()
    .match({ book_id: bookId, user_id: userId })

  if (error) throw new Error(error.message)

  return rewalist.filter(({ id }) => id !== String(bookId))
}
...
async function deleteVideo(
  ...
  const { error } = await supabase.from('user_watching_list')
    .delete()
    .match({ video_id: videoId, user_id: userId })

  if (error) throw new Error(error.message)

  return rewalist.filter(({ id }) => id !== String(videoId))
}
...
export function useReWaList(
  ...
  const removeFromRewalist = useCallback(
    async (item: ReWaListItemType) => {
      if (!userId) return
      const deleteFnc = item.type === 'book' ? deleteBook : deleteVideo
      await mutate(deleteFnc(+item.id, userId, data || []), {
        optimisticData: (data || []).filter(({ id }) => id !== item.id),
        populateCache: true,
      })
    },
    [data]
